...
class Solution {
public:
    bool is_anagram(const std::unordered_map<char, int>& window_map, const std::unordered_map<char, int>& anagram_map) {
        if (window_map.size() != anagram_map.size()) return false;

        for (auto& pair : window_map) {
            auto it = anagram_map.find(pair.first);
            if (it == anagram_map.end() || it->second != pair.second)
                return false;
        }
    ...
    bool hasSubstringAnagram(std::string s, std::string anagram) {
        ...
        while (j < (int)s.length()) {
            ...
            if (j - i + 1 == anagram.length()) {
                if (is_anagram(window_map, anagram_map)) return true;
                window_map[s[i]]--;
